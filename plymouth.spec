...
for i in bgrt charge glow spinfinity spinner; do
    cp %{buildroot}%{_datadir}/pixmaps/system-logo-white.png %{buildroot}%{_datadir}/plymouth/themes/$i/watermark.png
done

# (tpg) display something if BGRT image is empty
for i in bgrt spinner; do
    cp %{buildroot}%{_datadir}/pixmaps/system-logo-white.png %{buildroot}%{_datadir}/plymouth/themes/$i/bgrt-fallback.png
done
